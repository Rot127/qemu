...
static inline void log_load_gpr(uint32_t rx) {
    #ifdef HAS_TRACEWRAP
    if (rx == UINT32_MAX) {
        // The 0 register. In (RA|0) cases.
        #ifdef TARGET_PPC64
        TCGv_i64 o = tcg_const_i64(0);
        #else
...
static inline void log_load_spr(uint32_t spr, uint32_t field, TCGv val) {
    ...
    #else
    if (spr == SPR_XER && (field == XER_CA32 || XER_OV32)) {
                                      ...
                                      target_long maskl)
{
    ...
    simm &= ~maskl;
    if (rA(ctx->opcode) == 0) {
        // (RA|0) case. Log reg 0 read
        log_load_gpr(UINT32_MAX);
        if (NARROW_MODE(ctx)) {
            simm = (uint32_t)simm;
        }
